...
while read parameter_name parameter_value; do
  ...
  case ${parameter_name} in
    ...
    "SourceURL") export SOURCE_URL="$parameter_value";;
    "ScheduleCron") export SCHEDULE_CRON="$parameter_value";;
...
done < <(aws cloudformation describe-stacks --region $REGION --stack-name $CFN_STACK_NAME --query 'Stacks[0].Parameters' --output text$PROFILE)

# SCHEDULE_CRON="rate(1 day)"
echo "$SCHEDULE_CRON"
echo -----------------
...
zip -r pre-processing-code.zip . -x "*.dist-info/*" -x "bin/*" -x "**/__pycache__/*" )
...
aws lambda update-function-code --region $REGION --function-name $LAMBDA_FUNCTION_NAME --s3-bucket $S3_BUCKET --s3-key $DATASET_NAME/automation/pre-processing-code.zip$PROFILE
...
CFN_STACK_NAME="producer-${DATASET_NAME}-preprocessing"
#TODO: Currently does not automatically add rearc-data-utils layer again, need to be re-added manually in console
#TODO: Also: if configuration was changed (e.g. more RAM, longer timeout), needs to be changed manually in consule
#TODO: FIXIT: automate this
aws cloudformation update-stack --stack-name $CFN_STACK_NAME --template-body file://pre-processing/pre-processing-cfn.yaml --parameters ParameterKey=S3Bucket,ParameterValue=$S3_BUCKET ParameterKey=DataSetName,ParameterValue=$DATASET_NAME ParameterKey=DataSetArn,ParameterValue=$DATASET_ARN ParameterKey=ProductId,ParameterValue=$PRODUCT_ID ParameterKey=Region,ParameterValue=$REGION ParameterKey=SourceURL,ParameterValue="$SOURCE_URL" ParameterKey=ScheduleCron,ParameterValue="'$SCHEDULE_CRON'" --region $REGION --capabilities "CAPABILITY_AUTO_EXPAND" "CAPABILITY_NAMED_IAM" "CAPABILITY_IAM"$PROFILE
